...
export const purgeBullQueues = () => {
  ...
  const purgeQueues = map(queueNames, async (item) => {
    ...
    let clientDomain = queue.client.domain;
    let clientHost = queue.client.options.host;
    let clientIsLocal = clientHost == '127.0.0.1' || clientHost == 'localhost';

    if (clientDomain !== null || !clientIsLocal) {
      throw new Error(`Attempting to purge non local / queue, host was: ${clientHost}`);
    }
